...
- `db.py` — schema + queries. Tables: `readings`, `rates`, `standing_charges`, `weather_daily`, `meta` (key/value: `last_sync`, `{fuel}_serial`, `gas_unit`).
...
- `static/` — no-framework, no-library "Meterline" frontend (`app.js` + `index.html` + `style.css`): hand-rolled SVG/HTML charts, screens for usage/forecast/meters/tariff/settings, API responses memoized per page load. Served with `Cache-Control: no-cache`.
